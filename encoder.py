...
def Encode(key,message):            #encoding function
    enc=[]
...
def Decode(key,message):            #Decoding function
    dec=[]
...
def Mode():                         #fucntion to swap between encoding and decoding
    if(mode.get() == 'e'):
        Result.set(Encode(private_key.get(), Text.get()))
    elif(mode.get() == 'd'):
        Result.set(Decode(private_key.get(), Text.get()))
    else:
        Result.set('Invalid, enter "d" or "e"')
